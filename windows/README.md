...
Application coordination is under `src/app/`, audio lifecycle and device
selection are under `src/audio/`, and native Windows integration is isolated
under `src/platform/`.
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\audit-windows.ps1
```
...
See `SECURITY.md` for the microphone, integrity, and network boundaries.
