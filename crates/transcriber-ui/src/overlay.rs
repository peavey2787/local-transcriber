...
impl Overlay {
    ...
    pub fn reconcile_preferences(&mut self, config: &Config, preserve_notice: bool) {
        let allowed = match &self.state {
            OverlayState::Hidden => true,
            OverlayState::Loading { .. } => config.show_loading_notifications,
            OverlayState::Listening | OverlayState::CommandListening => {
                config.show_recording_notifications
            }
            OverlayState::Processing | OverlayState::CommandProcessing => {
                config.show_transcribing_notifications
            }
            OverlayState::Result { .. } => config.show_result_notifications,
            OverlayState::Notice { .. } => {
                preserve_notice
                    || config.show_loading_notifications
                    || config.show_result_notifications
            }
        };
        if !allowed {
            self.dismiss();
