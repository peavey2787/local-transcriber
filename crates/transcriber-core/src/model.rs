...
pub fn model_root(models_dir: &Path) -> PathBuf {
...
pub fn ensure_parakeet_int8<F>(models_dir: &Path, status: &F) -> Result<ModelPaths>
where
    F: Fn(String),
{
    status("Checking Parakeet model files…".into());
    let root = model_root(models_dir);
    ...
    let parent = models_dir.to_path_buf();
