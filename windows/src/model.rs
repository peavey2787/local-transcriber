...
const MODEL_ARCHIVE_NAME: &str = "sherpa-onnx-nemo-parakeet-tdt-0.6b-v3-int8.tar.bz2";
    ...
    F: Fn(String),
{
    ...
    log::info!(
        "extracting authenticated archive {}",
...
fn write_model_receipt(root: &Path) -> Result<()> {
    ...
    let temporary = root.join(format!("{MODEL_RECEIPT_NAME}.part"));
    {
        let mut file = File::create(&temporary).with_context(|| {
            format!(
                "create model authentication receipt {}",
                temporary.display()
            )
        })?;
        writeln!(file, "{MODEL_ARCHIVE_SHA256}").context("write model authentication receipt")?;
        file.sync_all().with_context(|| {
            format!("flush model authentication receipt {}", temporary.display())
        })?;
    }
    fs::rename(&temporary, &receipt).with_context(|| {
        format!(
            "activate model authentication receipt {}",
            receipt.display()
        )
    })?;
    Ok(())
...
fn extract_tar_bz2_safely(archive_path: &Path, parent: &Path, final_root: &Path) -> Result<()> {
    ...
    let extraction = (|| -> Result<()> {
        ...
        for entry in archive.entries().context("read model archive entries")? {
            ...
            let path = entry
                .path()
                .context("read model archive path")?
                .into_owned();
            ...
            {
                bail!(
                    "model archive entry escaped extraction root: {}",
                    path.display()
                );
            }
        }
        ...
        fs::rename(&extracted_root, final_root).with_context(|| {
            format!("activate verified model directory {}", final_root.display())
        })?;
        Ok(())
    })();
...
fn validate_archive_path(path: &Path) -> Result<()> {
    if path.is_absolute() {
        bail!(
            "model archive contains an absolute path: {}",
            path.display()
        );
    }
