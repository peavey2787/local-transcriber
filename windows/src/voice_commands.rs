...
fn validate_script_path(script: &str) -> Result<()> {
    ...
    if !matches!(extension.as_str(), "ps1" | "bat" | "cmd" | "py") {
        bail!("Windows voice commands support .ps1, .bat, .cmd, and .py files: {}", path.display());
    }
    Ok(())
}
...
fn execute_script(script: &str) -> Result<()> {
    ...
    let output = match extension.as_str() {
        "ps1" => Command::new("powershell.exe")
            ...
            .arg(path)
            .output(),
        "bat" | "cmd" => Command::new("cmd.exe").arg("/C").arg(path).output(),
        "py" => run_python(path),
...
fn run_python(path: &Path) -> std::io::Result<Output> {
    match Command::new("py.exe").arg("-3").arg(path).output() {
        Err(error) if error.kind() == ErrorKind::NotFound => {
            Command::new("python.exe").arg(path).output()
        }
        result => result,
    }
}
...
mod tests {
    ...
    fn matching_ignores_case_whitespace_and_surrounding_punctuation() {
        let commands = vec![VoiceCommand {
            ...
            scripts: vec![r"C:\scripts\report.ps1".into()],
        }];
        assert_eq!(
    ...
    fn duplicate_normalized_phrases_are_rejected_before_path_validation() {
        let commands = vec![
            VoiceCommand {
                ...
                scripts: vec!["missing.ps1".into()],
            },
            VoiceCommand {
                phrase: " build   app ".into(),
                scripts: vec!["other.ps1".into()],
            },
        ];
