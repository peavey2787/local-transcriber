...
pub const IDLE_ICON_COLOR: [u8; 4] = [0x1B, 0xB9, 0xCE, 0xFF];

pub fn mic_icon_rgba(size: u32) -> Vec<u8> {
    mic_icon_rgba_with_color(size, IDLE_ICON_COLOR)
}
...
mod tests {
    ...
    fn icon_has_the_requested_dimensions_and_transparency() {
        ...
        assert!(pixels.chunks_exact(4).any(|pixel| pixel == IDLE_ICON_COLOR));
    }
}
