//! Parakeet TDT v3 INT8 recognition via sherpa-onnx.
...
use crate::audio::{cpu_threads, trim_silence, SAMPLE_RATE};
...
impl AsrEngine {
    pub fn load_with_status<F>(models_dir: &Path, status: F) -> Result<Arc<Self>>
    where
        F: Fn(String),
    {
        let paths = ensure_parakeet_int8(models_dir, &status)?;
    ...
    pub fn transcribe_labeled(&self, audio: &[f32], label: Option<&str>) -> Result<String> {
        ...
        let real_time_factor = if audio_s > 0.0 { elapsed / audio_s } else { 0.0 };
        let tag = label.unwrap_or("parakeet-int8");
        println!(
            "[local-stt] {tag}: transcribed in {elapsed:.2}s (audio {audio_s:.1}s, RTF {real_time_factor:.2}x)"
        );
        Ok(text)
    }
}
